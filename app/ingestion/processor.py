...
logfire.configure(service_name="enterprise_ingestion_service")
...
async def eventarc_webhook(request: Request, background_tasks:BackgroundTasks):
    "Entry Point for Google Cloud Eventac Triggers"
    try:
        ...
        #2 Extract source_type from path
        parts=name.split('/')
        source_type=parts[0] if len(parts) > 1 else "general"
        filename=parts[-1]

        #3 Download to temp file and process in backgroud
...
def run_universal_ingestion(base_dir:str, explicit_source_type:str=None, wipe:bool=False):
    ...
    with logfire.span("Universal Ingestion Started", base_directory=base_dir):
        if wipe and qdrant_client.collection_exists(settings.QDRANT_COLLECTION):
            logfire.warning("Wiping existing collection")
...
if __name__=="__main__":
    ...
    if os.path.exists(target_dir):
        ...
        print(f"Error Path {target_dir} does not exists")
